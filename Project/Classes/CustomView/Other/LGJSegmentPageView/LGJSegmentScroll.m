...
@interface LGJSegmentScroll()<NSCacheDelegate, UIScrollViewDelegate>
{
...
@implementation LGJSegmentScroll
...
#pragma mark - setContentOffset
- (void)setContentOffset:(CGPoint)contentOffset animated:(BOOL)animated {
    [super setContentOffset:contentOffset animated:animated];
    NSInteger currentIndex = self.contentOffset.x/self.frame.size.width;
    if (!animated) {
        if ([self.segmentDelegate respondsToSelector:@selector(animationEndIndex:)]) {
            [self.segmentDelegate animationEndIndex:currentIndex];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if ([self.segmentDelegate respondsToSelector:@selector(scrollOffsetScale:)]) {
        [self.segmentDelegate scrollOffsetScale:scale];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    //滑动结束
    NSInteger currentIndex = self.contentOffset.x/self.frame.size.width;
    if ([self.segmentDelegate respondsToSelector:@selector(scrollEndIndex:)]) {
        [self.segmentDelegate scrollEndIndex:currentIndex];
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView {
    ...
    if ([self.segmentDelegate respondsToSelector:@selector(animationEndIndex:)]) {
        [self.segmentDelegate animationEndIndex:currentIndex];
    } else if (self.animationEnd)self.animationEnd(currentIndex);
}
