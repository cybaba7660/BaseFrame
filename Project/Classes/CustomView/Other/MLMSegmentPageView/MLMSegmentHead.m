...
- (void)titlesWidth {
    sum_width = (titlesArray.count - 1) * _singleW_Add + _paddingHoriz * 2;
    CGFloat width = (SCROLL_WIDTH - ((_maxTitles - 1) * _singleW_Add + _paddingHoriz * 2)) / _maxTitles;
...
- (CGFloat)titleWidth:(NSString *)title {
    UIFont *font = _fontScale > 1 ? [UIFont fontWithName:_font.fontName size:_font.pointSize*_fontScale] : _font;
    return [title boundingRectWithSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(self.frame)) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : font} context:nil].size.width;
}
...
- (void)scrollViewSubviews:(UIScrollView*)scroll {
    ...
    if (titles && _headStyle != SegmentHeadStyleSlide) {
        ...
        if (_fontScale != 1) {
            curBtn.titleLabel.font = [UIFont fontWithName:_font.fontName size:_font.pointSize*_fontScale];
        }
...
- (void)changeBtnFrom:(NSInteger)from to:(NSInteger)to {
    ...
    if (_fontScale) {
        before_btn.titleLabel.font = _font;
        select_btn.titleLabel.font = [UIFont fontWithName:_font.fontName size:_font.pointSize*_fontScale];;
    }
...
- (void)fontChangeCurBtn:(UIButton *)curBtn nextBtn:(UIButton *)nextBtn changeScale:(CGFloat)changeScale{
    //button字体改变的大小
    CGFloat btn_font_change = _font.pointSize*(_fontScale - 1);
    //大小变化
    CGFloat next_font = _font.pointSize + changeScale*btn_font_change;
    CGFloat cur_font = _font.pointSize*_fontScale - changeScale*btn_font_change;
    nextBtn.titleLabel.font = [UIFont fontWithName:_font.fontName size:next_font];
    curBtn.titleLabel.font = [UIFont fontWithName:_font.fontName size:cur_font];
}
